...
- `src/week_data.py` — the pure per-week transforms the fetcher runs on each
  ...
  projected totals, leaders, and the week's projected standings) and the rule
  deciding whether a run publishes what it fetched at all.
...
needs no credentials. The week fixtures are written to the week
