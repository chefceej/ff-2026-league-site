...
test.describe("Projected standings block", () => {
  ...
  test("lists every team in the week file's order", async ({ page }) => {
    const live = loadWeekFixture("in-progress");
    await openScoreboard(page, { weeks: { 14: live } });

    await expect(blockRows(page)).toHaveCount(live.projected_standings.length);
    await expect(page.locator("#projected-table td.team"))
      .toHaveText(live.projected_standings.map(r => r.team_name));
    // The file orders the teams by projected total; the page must not re-sort.
    const totals = live.projected_standings.map(r => r.projected_total);
    expect([...totals].sort((a, b) => b - a)).toEqual(totals);
  });

  test("shows each team's projected rank, total, ranking points and normalized",
       async ({ page }) => {
    const live = loadWeekFixture("in-progress");
    await openScoreboard(page, { weeks: { 14: live } });
    const rows = live.projected_standings;
  ...
  test("marks a normalized total above the cutoff apart from one below",
       async ({ page }) => {
    const live = loadWeekFixture("in-progress");
    await openScoreboard(page, { weeks: { 14: live } });
    const rows = live.projected_standings;
  ...
  test("draws the Standings table's own movement indicator", async ({ page }) => {
    const live = loadWeekFixture("in-progress");
    await openScoreboard(page, { weeks: { 14: live } });
    const rows = live.projected_standings;
    ...
    await expect(page.locator("#projected-table .mv.down"))
      .toHaveCount(rows.filter(r => move(r) < 0).length);
    // The fixture has to contain a mover each way for that to mean anything.
